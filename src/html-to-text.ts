...
import { HtmlToTextOptions, HtmlToTextRunOptions } from './interfaces';

// Which type of tags should not be parsed
const SKIP_TYPES = ['style', 'script'];

function htmlToText(html: string, options?: HtmlToTextOptions) {
  const finalOptions: HtmlToTextRunOptions = Object.assign(
    {
      wordwrap: 80,
      tables: [],
      ...
      lineCharCount: 0 //XXX This should be handled differently since it is not a "real" option
    },
    options || {}
  );
...
function filterBody(dom: any, options: HtmlToTextRunOptions, baseElement: any) {
  let result: any = null;
...
function walk(dom: any, options: HtmlToTextRunOptions, result?: string) {
      ...
      case 'tag':
        switch (elem.name.toLowerCase()) {
          ...
          case 'ol':
            ...
            break;
          case 'pre':
            const newOptions: HtmlToTextRunOptions = Object.assign({}, options);
