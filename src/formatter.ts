...
import { compact, get, max, padEnd, times } from 'lodash';

import helper from './helper';

function formatText(elem: any, options: any) {
  let text: string = elem.data || '';
  ...
  if (options.isInPre) {
    ...
    return helper.wordwrap(
      elem.trimLeadingSpace ? text.trimLeft() : text,
      options
    );
  }
}
...
function formatAnchor(elem: any, fn: Function, options: any) {
  ...
  let text: string = fn(elem.children || [], options);
  if (!text) {
    text = '';
  }
